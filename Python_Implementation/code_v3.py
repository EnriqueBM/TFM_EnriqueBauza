...
def mute_monoms_4(ind):
    ...
    if (len(ind_dict.keys()) != 0):
        ...
        monom = tuple(monom)
        #coeff = random.choice([-2, -1, 1, 2])
        coeff = 2.0 * random.uniform(-1.0, 1.0)
...
def mute_coeffs(ind):
    ...
    for key in ind_dict.keys(): #the key is the monom, the value the coeff
        if (random.uniform(0, 1) < rate1) and not(muted):
            ...
            coeff_mut = 4.0 * random.uniform(-1.0, 1.00)
...
def fitness_mae(ind, obj_ev, samps):
    n_samps = len(samps)
...
if __name__ == "__main__":
    ...
    for iter in range(n_iter):
        ...
        if (iter % 1000 == 0):
            ...
            f = lambdify(x, next_pop[0].as_expr(), "numpy")
            evals = f(samps)
            #print(type(evals))
            ymax = np.amax(evals)
            ymin = np.amin(evals)
