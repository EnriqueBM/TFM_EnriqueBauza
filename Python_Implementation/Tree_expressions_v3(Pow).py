...
def gen_branch_2(depth, max_args):
    ...
    if func != Pow:
        ...
        if depth == 1:
            largs = [gen_leaf() for auxi in range(n_args-1)]
            largs.append(sympify(random.randint(-2, 2)))
            return func(*largs, evaluate=False)
        else:
            largs = [gen_branch_2(depth-1, max_args) for auxi in range(n_args-1)]
            largs.append(sympify(random.randint(-2, 2)))
            return func(*largs, evaluate=False)
...
def mute_1(expr):
    ...
    if len(expr.args) != 0: #Por si el individuo se ha simplificado demasiado
        """if expr.func != Pow:
            ...
            idx = 0""" # Si es potencia se cambia la base
        idx = random.choice(range(len(expr.args)))
        if len(expr.args[idx].args) == 0: ## Node before leaf
            largs = list(expr.args)
            largs[idx] = gen_branch_2(1, 2)
...
def mute_3(expr):
    ...
    """
    if len(expr.args) != 0: ## Un individuo que es solo 1 nodo
        idx = random.choice(range(len(expr.args)))
        if len(expr.args[idx].args) == 0: ## Node before leaf, replace a leaf
            if expr.func != Pow:
                ...
                if idx == 0: ## Base de la potencia
                    largs = list(expr.args)
                    largs[idx] = gen_leaf()
                else: # El exponente siempre debe ser un entero
                    largs = list(expr.args)
                    largs[idx] = largs[idx] + sympify(random.randint(-2, 2))
            return expr.func(*largs, evaluate=False)
        elif random.uniform(0, 1) < node_mutation_prob: ## Replace the node function
            largs = list(expr.args)
            func = random.choice(branch_list) #Nueva funcion
            if func != Pow:             
                if arity(func) != None: # La nueva no es Add ni Mul
